...
class ClickStreamGen(object):
    def __init__(self, num_users, clickstream_data_path, clustered_article_data_path):
        ...
        for cluster_id in np.unique(self.clustered_article_data.loc[:, "cluster_id"]):
            cluster_articles = self.clustered_article_data.loc[
                self.clustered_article_data.loc[:, "cluster_id"] == cluster_id,
                "article_id",
            ].values
            article_prob_hash = {}
    ...
    def generate(self):
        ...
        (
            self.user_ids,
            self.session_ids,
            self.user_num_sessions_hash,
        ) = self._generate_user_ids_session_ids()
        ...
        for list1, list2 in combinations(lists_hash.keys(), 2):
            assert len(lists_hash[list1]) == len(
                lists_hash[list2]
    ...
    def _generate_article_ids(self):
        ...
        for user_id in self.user_num_sessions_hash.keys():
            ...
            for article_id in list(user_preference_article_ids):
                self.cluster_article_prob_hash[user_preference_cluster_id][
                    article_id
                ] += 1
    ...
    def get_cluster_article_ids(
        ...
        user_preference_article_ids = random_nonuniform_gen.choice(
            ...
            p=np.array(list(article_prob_hash.values()))
            / sum(article_prob_hash.values()),
        )
    ...
    def _generate_click_time_spent(self):
        ...
        time_spent = []
        generator = tqdm(
            zip(self.user_ids, self.session_ids, self.article_ids, self.ranks),
        ...
        for data in generator:
            ...
            random_uniform_gen = np.random.default_rng()
            is_click = random_uniform_gen.choice(a=[1, 0], p=[0.7, 0.3])
            if is_click:
                click.append(1)
