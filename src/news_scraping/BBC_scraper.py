...
def make_disallowed_list(disallow_string: str, homepage_url: str) -> list:
    ...
    """
    disallowed_hrefs = [homepage_url + str(href[len("disallow: "):])
                        for href in disallow_string.strip().split('\n')]
...
class BBCNewsScraper(NewsScraper):
    ...
    def scrape_navigation_bar(self):
        ...
        nav_bar = driver.find_element_by_xpath(
            "//ul[@class='gs-o-list-ui--top-no-border nw-c-nav__wide-sections']")
        preview_hrefs_this_page = nav_bar.find_elements_by_class_name(
            "nw-o-link")
        preview_hrefs = [tag.get_attribute("href")
                         for tag in preview_hrefs_this_page]
    ...
    def scrape_article_links(self):
        ...
        for preview_href in self.preview_hrefs:
            if not(preview_href in self.disallowed_urls):
                print(
                    f"[{self.__class__.__name__}] finding article links in: {preview_href}")
                ...
                article_hrefs_this_page = [
                    tag.get_attribute("href") for tag in href_elements]
    ...
    def scrape_article_contents(self):
        news_data = pd.DataFrame(
            columns=["url", "title", "raw_text", "author", "publish_datetime"])
        driver = self.driver
        ...
        for article_index, article_url in enumerate(tqdm(self.article_hrefs)):
            ...
            if article_url not in self.disallowed_urls:
                ...
                try:
                    title = soup.find(
                        lambda tag: tag.name == 'h1' and tag['class'] == "ssrcss-1pl2zfy-StyledHeading e1fj1fc10".split(" "))
                    title = title.text.strip()
                except:
                    print(
                        f"[{self.__class__.__name__}] title not found, skipping")
                    continue
                # raw_text
                try:
                    raw_text = soup.find_all(
                        class_="ssrcss-18snukc-RichTextContainer e5tfeyi1")
                    raw_text = "".join(
                        [str(raw_text_.text) + " " for raw_text_ in raw_text])
                except:
                    raw_text = ""
                # author
                try:
                    author = soup.find(
                        class_="ssrcss-1pjc44v-Contributor e5xb54n2").text
                except:
                    author = "bbc"
                # publish_date
                try:
                ...
                news_data = news_data.append(
                    this_article_data, ignore_index=True)
