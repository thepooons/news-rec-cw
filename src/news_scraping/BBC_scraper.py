...
def make_disallowed_list(disallow_string: str, homepage_url: str) -> list:
    ...
    """
    disallowed_hrefs = [
        homepage_url + str(href[len("disallow: ") :])
...
class BBCNewsScraper(NewsScraper):
    def __init__(
        ...
        self.disallowed_urls = make_disallowed_list(
            disallow_string=disallow_string, homepage_url=homepage_url
        )
        self.article_hrefs = []
            ...
            if article_url not in self.disallowed_urls:
                ...
                try:
                    title = soup.find(
                        ...
                        and tag["class"]
                        == "ssrcss-1pl2zfy-StyledHeading e1fj1fc10".split(" ")
                    )
                ...
                try:
                    author = soup.find(
                        class_="ssrcss-1pjc44v-Contributor e5xb54n2"
                    ).text
                except:
                    author = "bbc"
                # publish_date
                ...
                this_article_data = pd.DataFrame(
                    data={
                        "url": [article_url],
                        "title": [title],
                        "raw_text": [raw_text],
